	...
	<el-dialog v-model="visible" :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false" draggable>
		<el-form ref="dataFormRef" :model="dataForm" :rules="dataRules" label-width="120px" @keyup.enter="submitHandle()">
			<el-form-item prop="parkId" label="所属停车场">
				<el-select v-model="dataForm.parkId" class="m-2" placeholder="所属小区" @change="updateCommunityName">
					<el-option v-for="item in communityList" :key="item.id" :label="item.parkyName" :value="item.id" />
				</el-select>
			</el-form-item>

			<el-form-item prop="ownerId" label="所属楼宇">
				<el-select v-model="dataForm.ownerId" class="m-2" placeholder="所属楼宇" @change="updateBuildingName">
					<el-option v-for="item in filteredBuildings" :key="item.id" :label="item.realName" :value="item.id" />
				</el-select>
			</el-form-item>

			<el-form-item prop="carportName" label="房间号">
				<el-input v-model="dataForm.carportName" placeholder="车位号"></el-input>
			</el-form-item>
