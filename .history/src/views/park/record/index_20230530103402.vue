<template>
	<el-card>
		<el-form :inline="true" :model="state.queryForm" :data="state.dataListWithDuration" @keyup.enter="getDataList()">
			<el-form-item>
...
const state: IHooksOptions = reactive({
    ...
    const dataListWithDuration = () => {
	return this.dataList.map((item: any) => {
		const entranceTime = new Date(item.accessTime).getTime()
		const exitTime = new Date(item.leaveTime).getTime()
		const durationMs = exitTime - entranceTime
